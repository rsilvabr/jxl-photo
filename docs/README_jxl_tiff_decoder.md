...
#           and NEVER over an original master (see "Original TIFF masters
...
## Original TIFF masters are never overwritten

The encoder's default mode 0 leaves `photo.tif` and `photo.jxl` **side by
side**. The JXL is newer, so a plain sync decode into the same folder used to
see "JXL newer than TIFF" and **overwrite the original master** with the
decode — lossy, if the archive was lossy — without any delete flag involved.

The decoder now tells its own outputs apart: every TIFF it writes carries the
`jxlphoto-src`/`jxlphoto-srcsum` provenance marker. In the default smart sync
an existing TIFF **without** that marker is treated as an original and is
...
- A refused file is **not** a skip: its JXL is not decoded, and it is never
  ...
  is not that JXL's decode, so it proves nothing about it).
- Refusals are listed at the end of the run and reported to the wrapper
  (`Refused (existing TIFF is an original master)`); the exit code is
  unchanged — refusing is the intended, safe outcome.
- `--dry-run` previews them (`would REFUSE`).
- To decode such a folder, write somewhere else (`--mode 1`, `--mode 3`, an
  output folder), or pass `--overwrite` (without `--sync`) if replacing those
  TIFFs is really what you want.
...
Options:
  ...
  --overwrite         Always overwrite existing TIFFs — including an
                      original master the default sync refuses to touch
  --sync              Reconvert only JXLs newer than their TIFF, and never
                      over a TIFF this tool did not decode
...
- **eciRGB v2, scanner profiles and other table-curve ICC profiles**: a lossy JXL whose profile has no native form (table curve) stores the whole ICC, and djxl decodes it to linear sRGB. Files the encoder wrote with its default `cautious` strategy as "skip" (eciRGB v2 and Epson scanner profiles, verified) round-trip correctly. A lossy JXL that *does* carry such an ICC blob is a **known open bug** in Roundtrip mode: the original ICC is pasted on linear-sRGB pixels and the colours come out wrong. Check it with djxl's `icc_out` and `orig_icc_out` options (write both profiles for the same file): if the two files differ, keep the TIFF until the fix lands. Details: [Lossy JXL with an ICC blob](jxl_color_internals.md#lossy-jxl-with-an-icc-blob-what-djxl-returns-measured-2026-09-25).
