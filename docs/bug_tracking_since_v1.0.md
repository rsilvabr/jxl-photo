...
4. **Grayscale and SubfileType preservation**
   ...
   - `jxl_tiff_decoder.py` restores grayscale pages as 2D single-channel TIFFs and restores non-zero `SubfileType` values, `SubfileType=4` (MASK) included — a film scanner's IR page round-trips `4 → 4`. *(As written in v1.7.0 this said MASK is mapped to `PAGE` (`2`), because tifffile's `subfiletype=` parameter accepts only `0`/`1`/`2`; such values are written as the raw TIFF tag 254 instead. Corrected here in v2.0.1 — see #300 — rather than left standing as a historical record.)*
