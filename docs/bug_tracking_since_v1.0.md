...
Users running from manifest would silently lose these features.
...
**Problem:** The manifest builder was not passing several flags that `execute_workflow()` expected:
- `--staging`
- `--encode-tag`
- `--d50-patch`
...
**Fix:** Added all missing flags to the manifest command builder:
```python
...
**Location:** `jxl_photo.py` lines ~2590

**Problem:** The transcoder command was adding `--format` twice — once in the main section and once in the dest-specific section.

**Fix:** Consolidated all `--format` logic into one place, removed duplicate.
...
**Problem:** The thumbnail fallback code had `return` without checking if PIL was available, causing `NameError` when PIL import failed.

**Fix:** Added check for PIL availability:
```python
if 'Image' not in globals():
    logger.debug("  >PIL not available, skipping thumbnail fallback")
    return
...
All 58 tracked bugs have been fixed. The toolkit is now robust and ready for v1.5 release.

**Key improvements in v1.5:**
- Auto mode for JXL→JPEG directories
- Configurable TIFF preview
- Proper --decode flags for all conversion types
